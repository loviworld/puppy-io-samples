!function (factory) {
  if (typeof require === 'function' && typeof module !== 'undefined') {
    ...
    if(!SockJS) {
      throw new Error('puppy-io-ui.js requires sockjs-client, see http://sockjs.org');
    }
    factory(SockJS);
  } else if (typeof define === 'function' && define.amd) {
    // AMD loader
    define('puppy-io-ui', ['sockjs'], factory);
  } else {
    // plain old include
    ...
    ServiceListener = factory(this.SockJS);
  }
}(function (SockJS) {
   ...
   * @param {Snteger} serverPort
   * @param {Array} options
   */
  var ServiceListener = function(appName, serverPort, options){
	  this.serverAddress = "localhost";
	  this.serverPort = serverPort;
	  this.appName = appName;
	  var url = 'http://' + this.serverAddress + ':' + this.serverPort + '/ui.' + this.appName;
	  this.EventBusCon = new EventBus(url, options);
  };
   ...
   * @param {Function} callback
   */
  ServiceListener.prototype.onopen = function (callback) {
	  this.EventBusCon.onopen = callback;
  };
   ...
   * @param {Function} callback
   */
  ServiceListener.prototype.send = function (serviceMethod, message, headers, callback) {
	  var address = this.appName + "." + serviceMethod;
	  this.EventBusCon.send(address, message, headers, callback);
  };
   ...
   * @param {Object} headers
   */
  ServiceListener.prototype.publish = function (serviceMethod, message, headers) {
	  var address = this.appName + "." + serviceMethod;
	  this.EventBusCon.publish(address, message, headers);
  };
  ...
  if (typeof exports !== 'undefined') {
    if (typeof module !== 'undefined' && module.exports) {
      exports = module.exports = ServiceListener;
    } else {
      exports.ServiceListener = ServiceListener;
    }
  } else {
